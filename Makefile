...
write: init
	pnpm -r exec eslint --fix && pnpm prettier -w .
	uv run ruff check --fix --no-cache && uv run ruff format --no-cache

check: init
	pnpm -r exec eslint && pnpm prettier -c .
	uv run ruff check --no-cache && uv run ty check --error-on-warning
