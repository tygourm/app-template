...
maintained by Vercel. Contains 67 rules across 8 categories, prioritized by
...
- `async-cheap-condition-before-await` - Check cheap sync conditions before
  awaiting flags or remote values
...
- `server-parallel-nested-fetching` - Chain nested fetches per item in
  Promise.all
...
- `rerender-use-deferred-value` - Defer expensive renders to keep input
  responsive
